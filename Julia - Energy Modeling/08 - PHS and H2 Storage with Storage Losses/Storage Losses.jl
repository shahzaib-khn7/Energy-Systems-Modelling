...
@constraint(ESM, StorageLevelFunction[s in storages, h in hour, f in fuels; h>1 && StorageDischargeEfficiency[s,f]>0], 
    StorageLevel[s,h,f] == StorageLevel[s,h-1,f]*StorageLosses[s] + StorageCharge[s,h,f]*StorageChargeEfficiency[s,f] - StorageDischarge[s,h,f]/StorageDischargeEfficiency[s,f]
)
...
@constraint(ESM, MaxStorageCapacityFunction[s in storages],
    sum(StorageEnergyCapacity[s, ff] for ff in fuels if StorageDischargeEfficiency[s, ff] > 0) == MaxStorageCapacity[s]
)
